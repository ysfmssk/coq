...
Theorem Inf_cons: forall f l p, LPInf l f -> LPInf (p::l) f. Proof. intros. apply Inf_incl with l; auto. intros x Hx; right; auto. Qed.
...
Theorem assign_prep_spec5: forall l a, NoDup l -> exists p, In p (assign_prep l) /\ forall n, In n l->a n=fst p n. Proof. intros. induction H; simpl; intros. exists (fun _=>false, nil). split; auto. intros. destruct H. destruct (IHNoDup) as [[b r] [H1 H2]]. simpl in H2. remember (a x) as c. destruct c. exists (fun y=>if nat_eq_dec y x then true else b y, (#x)::r). split. apply in_or_app. left. apply in_map_iff. exists (b,r); auto. intros. simpl. destruct H3. subst x. destruct (nat_eq_dec n n); auto. contradict n0; auto. destruct (nat_eq_dec n x). subst x. auto. apply H2; auto. exists (b, (! #x)::r). split. apply in_or_app. right. apply in_map_iff. exists (b,r); auto. simpl. intros. destruct H3. subst x. rewrite <- Heqc. rewrite <- assign_prep_spec2 with l n (b,r); auto. auto. Qed.
