...
Theorem ExAny_AnyEx: forall pre x y f, LQInf pre (Ex x (Any y f)) -> LQInf pre (Any y (Ex x f)). Proof. intros. apply LQIMp with (Ex x (Any y f)) nil pre; auto. apply InfTh1. apply LQIUg. intros. destruct H0. subst p. intros C. inversion C. inversion H1. inversion H5. inversion H7. contradict H10; auto. destruct H0. destruct (nat_eq_dec y x). subst y. apply LQIEi with x x (Any x f) (Any x f); auto. intros. destruct H0. subst p. intros C. inversion C. inversion H1. contradict H4; auto. destruct H0. intros C; inversion C. contradict H2; auto. intros C. inversion C. inversion H1. contradict H4; auto. apply InfTh1. apply LQIEg with (Var x) f; auto. apply LQIUi with x (Var x) f; auto.
  destruct (varsIn f) as [l H0]. destruct (ubound_sig (x::y::l)) as [z H1 H2]. assert (z<>x). intros C. subst x. absurd (z<z); auto. assert (z<>y). intros C. subst y. absurd (z<z); auto. assert (~In z l). intros C. absurd (z<z); auto. clear H1 H2. destruct (tffSub x (Var z) f) as [[f' H1 H2]|H1]. apply LQIEi with x z (Any y f) (Any y f'); auto. intros. destruct H6. subst p. intros C. apply FreeVar_VarIn in C. inversion C. inversion H7. inversion H9. inversion H12. apply H5. apply H0; auto. contradiction. contradiction. destruct H6. intros C. inversion C. contradict H5; apply H0; auto. intros C. inversion C. inversion H7. inversion H11. contradict H5; apply H0; auto.
  inversion H1. subst f0 g. apply TFFSub_intro; auto. intros. inversion H8. subst i. intros C. apply BBR_VarIn in C. inversion C. contradict H5; apply H0; auto. contradiction. apply InfTh1. apply LQIEg with (Var z) f'; auto. apply LQIUi with y (Var y) f'; auto.
  destruct (sub x (Var z) f) as [g H6 H7]. absurd (TFFSub x (Var z) f g); auto. apply TFFSub_intro; auto. intros. intros C. inversion H2. subst i. apply BBR_VarIn in C. contradict H5. apply H0; auto. Qed.

Theorem AnyAnd_dist1: forall pre v f g, LQInf pre (Any v f &^ Any v g) -> LQInf pre (Any v (f &^ g)). Proof. intros. apply LQIMp with (Any v f &^ Any v g) nil pre; auto. apply InfTh1. clear H. apply LQIUg. intros. destruct H. subst p. intros C. inversion C. subst f0. inversion H0. subst f0 g0. inversion H1; contradict H3; auto. inversion H1. subst f0 g0 f1. inversion H4; contradict H3; auto. destruct H. apply AndIntro. apply InfMp with (Any v f). apply InfTh1. apply LQIUi with v (Var v) f; auto. eapply AndElim1; eauto. apply InfMp with (Any v g); auto. apply InfTh1. apply LQIUi with v (Var v) g; auto. eapply AndElim2; eauto. Qed.
Theorem AnyAnd_dist2: forall pre v f g, LQInf pre (Any v (f &^ g)) -> LQInf pre (Any v f &^ Any v g). Proof. intros. apply AndIntro. apply LQIMp with (Any v (f&^g)) nil pre; auto. apply InfTh1. apply LQIUg. intros. destruct H0. subst p. intros C. inversion C. contradict H2; auto. destruct H0. apply InfMp with (f &^g). apply InfTh1. eapply AndElim1; eauto. apply LQIUi with v (Var v) (f&^g); auto. apply LQIMp with (Any v (f&^g)) nil pre; auto. apply InfTh1. apply LQIUg. intros. destruct H0. subst p. intros C. inversion C. contradict H2; auto. destruct H0. apply InfMp with (f&^g). apply InfTh1. eapply AndElim2; eauto. apply LQIUi with v (Var v) (f&^g); auto. Qed.
Theorem AnyOr_dist1: forall pre v f g, LQInf pre (Any v f |^ Any v g) -> LQInf pre (Any v (f |^ g)). Proof. intros. apply LQIMp with (Any v f |^ Any v g) nil pre; auto. apply InfTh1. apply LQIUg. intros. destruct H0. subst p. intros C. inversion C. inversion H1. inversion H4. contradict H7; auto. inversion H1. contradict H5; auto. destruct H0. apply OrElim with (Any v f) (Any v g); auto; apply InfTh1; [apply OrIntro1|apply OrIntro2]; eapply LQIUi with (v:=v) (t:=Var v); eauto. Qed. 
Theorem ExAnd_dist1: forall pre v f g, LQInf pre (Ex v (f &^g)) -> LQInf pre (Ex v f &^ Ex v g). Proof. intros. apply LQIMp with (Ex v (f&^g)) nil pre; auto. apply InfTh1. destruct (varsIn (f&^g)) as [l H0]. destruct (ubound_sig (v::l)) as [x H1 _]. destruct (tffSub v (Var x) f) as [[f' H2 _]|H2]. destruct (tffSub v (Var x) g) as [[g' H3 _]|H3]. apply LQIEi with v x (f &^g) (f' &^g'); auto. intros. destruct H4. subst p. intros C. apply FreeVar_VarIn in C. inversion C. inversion H5. inversion H7. absurd (x<x); auto. apply H1. right. apply H0; auto. subst v. absurd (x<x); auto. destruct H4. intros C. absurd (x<x); auto. apply H1. right. apply H0; auto. intros C. absurd (x<x); auto. apply H1. apply FreeVar_VarIn in C. inversion C. inversion H5. inversion H7. inversion H10. right. apply H0. inversion H12.  unfold LQand; auto. auto. inversion H7. inversion H10. inversion H12; auto. inversion H14. right. apply H0. unfold LQand; auto.
